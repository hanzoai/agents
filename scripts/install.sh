#!/usr/bin/env bash
# Hanzo Agents CLI Installer
# Usage:
#   Production:  curl -fsSL https://hanzo-agents.ai/install.sh | bash
#   Staging:     curl -fsSL https://hanzo-agents.ai/install.sh | bash -s -- --staging
#   Version pin: VERSION=v1.0.0 curl -fsSL https://hanzo-agents.ai/install.sh | bash

set -e

# Configuration
REPO="hanzoai/hanzo-agents"
VERBOSE="${VERBOSE:-0}"
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --verbose|-v)
        ...
        ;;
      --help|-h)
        echo "Hanzo Agents CLI Installer"
        echo ""
        echo "Usage:"
        echo "  curl -fsSL https://hanzo-agents.ai/install.sh | bash"
        echo "  curl -fsSL https://hanzo-agents.ai/install.sh | bash -s -- --staging"
        echo ""
        echo "Options:"
        ...
        echo "  HANZO_AGENTS_INSTALL_DIR  Custom install directory"
        exit 0
        ;;
      *)
...
set_install_dir() {
  if [[ "$STAGING" == "1" ]]; then
    INSTALL_DIR="${HANZO_AGENTS_INSTALL_DIR:-$HOME/.hanzo-agents-staging/bin}"
    SYMLINK_NAME="af-staging"
  else
    INSTALL_DIR="${HANZO_AGENTS_INSTALL_DIR:-$HOME/.hanzo-agents/bin}"
    SYMLINK_NAME="af"
  fi
}
...
print_banner() {
  ...
  local title="Hanzo Agents CLI Installer"

  if [[ "$STAGING" == "1" ]]; then
    title="Hanzo Agents CLI Installer (STAGING)"
  fi
  ...
  if [[ "$STAGING" == "1" ]]; then
    ...
    printf "${YELLOW}For production use: curl -fsSL https://hanzo-agents.ai/install.sh | bash${NC}\n"
    printf "\n"
  else
    printf "${CYAN}╔%s╗${NC}\n" "$horizontal_line"
...
install_binary() {
  ...
  # Copy binary
  cp "$binary_path" "$install_dir/hanzo-agents"
  chmod +x "$install_dir/hanzo-agents"

  # Create symlink for convenience (best effort)
  local symlink_created=0
  if ln -sf "$install_dir/hanzo-agents" "$install_dir/$SYMLINK_NAME"; then
    symlink_created=1
    print_verbose "Created symlink: $SYMLINK_NAME -> hanzo-agents"
  else
  ...
  if [[ "$(detect_os)" == "darwin" ]]; then
    ...
    xattr -d com.apple.quarantine "$install_dir/hanzo-agents" 2>/dev/null || true
    if [[ "$symlink_created" -eq 1 ]]; then
      xattr -d com.apple.quarantine "$install_dir/$SYMLINK_NAME" 2>/dev/null || true
    fi
  fi

  print_success "Binary installed to $install_dir/hanzo-agents"
  if [[ "$symlink_created" -eq 1 ]]; then
    print_success "Symlink created: $install_dir/$SYMLINK_NAME"
  else
    print_info "You can create a manual shortcut named '$SYMLINK_NAME' pointing to $install_dir/hanzo-agents if desired."
  fi
}
...
configure_path() {
  ...
  local comment="# Hanzo Agents CLI"

  if [[ "$STAGING" == "1" ]]; then
    comment="# Hanzo Agents CLI (STAGING)"
  fi
  ...
  printf "\n"
  print_info "To use hanzo-agents in this terminal session, run:"
...
verify_installation() {
  ...
  if [[ -x "$install_dir/hanzo-agents" ]]; then
    print_success "Installation verified"

    # Try to get version
    if "$install_dir/hanzo-agents" --version >/dev/null 2>&1; then
      local version_output
      version_output=$("$install_dir/hanzo-agents" --version 2>&1)
    ...
    print_error "Binary not found or not executable: $install_dir/hanzo-agents"
    exit 1
  fi
}
...
print_success_message() {
  ...
  if [[ "$STAGING" == "1" ]]; then
    printf "${YELLOW}╔══════════════════════════════════════════════════════════════╗${NC}\n"
    printf "${YELLOW}║${NC}  ${BOLD}Hanzo Agents CLI (STAGING) installed successfully!${NC}            ${YELLOW}║${NC}\n"
    printf "${YELLOW}╚══════════════════════════════════════════════════════════════╝${NC}\n"
    printf "\n"
    ...
    printf "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}\n"
    printf "${GREEN}║${NC}  ${BOLD}Hanzo Agents CLI installed successfully!${NC}                      ${GREEN}║${NC}\n"
    printf "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}\n"
  fi
  ...
  printf "  Documentation: ${BLUE}https://hanzo-agents.ai/docs${NC}\n"
...
main() {
  ...
  local binary_name="hanzo-agents-$os-$arch"
  if [[ "$os" == "windows" ]]; then
    binary_name="hanzo-agents-$os-$arch.exe"
  fi
