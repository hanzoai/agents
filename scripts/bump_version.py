...
PY_INIT_FILE = REPO_ROOT / "sdk/python/hanzo_agents/__init__.py"
PKG_INFO_FILE = REPO_ROOT / "sdk/python/hanzo_agents.egg-info/PKG-INFO"
    ...
    for path in REQUIREMENT_FILES:
        ...
        for idx, line in enumerate(lines):
            if line.strip().startswith("hanzo_agents"):
                ...
                break
        if not replaced:
            raise RuntimeError(f"Failed to update hanzo_agents pin in {path}")
        write_file(path, "\n".join(lines) + "\n")
...
def update_go_template(version: SemVer) -> None:
    ...
    new_text, count = re.subn(
        r"(require\s+github\.com/hanzoai/agents/sdk/go\s+v)[0-9A-Za-z\.-]+",
        rf"\g<1>{version}",
        text,
        count=1,
    )
