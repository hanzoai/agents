# Hanzo Agents CLI Installer for Windows
# Usage: iwr -useb https://hanzo-agents.ai/install.ps1 | iex
# Version pinning: $env:VERSION="v1.0.0"; iwr -useb https://hanzo-agents.ai/install.ps1 | iex

$ErrorActionPreference = "Stop"

# Configuration
$Repo = "hanzoai/agents"
$InstallDir = if ($env:HANZO_AGENTS_INSTALL_DIR) { $env:HANZO_AGENTS_INSTALL_DIR } else { "$env:USERPROFILE\.hanzo-agents\bin" }
...
function Write-Banner {
    ...
    Write-Host "║           Hanzo Agents CLI Installer (Windows)             ║" -ForegroundColor Cyan
    Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
}
...
function Install-Binary {
    ...
    # Copy binary
    $targetPath = Join-Path $InstallDir "hanzo-agents.exe"
    ...
    try {
        ...
        Write-Verbose "Created hardlink: af.exe -> hanzo-agents.exe"
    }
    catch {
    ...
    if ($aliasCreated) {
        ...
        Write-Info "Alias not created; run hanzo-agents using $targetPath or create your own shortcut."
    }
}
...
function Test-Installation {
    ...
    $binaryPath = Join-Path $InstallDir "hanzo-agents.exe"
...
function Write-SuccessMessage {
    ...
    Write-Host "║  Hanzo Agents CLI installed successfully!                      ║" -ForegroundColor Green
    Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "     hanzo-agents --version" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  3. Initialize your first agent:" -ForegroundColor White
    Write-Host "     hanzo-agents init my-agent" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Resources:" -ForegroundColor White
    Write-Host "  Documentation: https://hanzo-agents.ai/docs" -ForegroundColor Blue
...
function Main {
    ...
    $binaryName = "hanzo-agents-$os-$arch.exe"
    ...
    $tempDir = Join-Path $env:TEMP "hanzo-agents-install-$(Get-Random)"
