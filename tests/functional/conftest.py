"""
Pytest configuration and fixtures for Hanzo Agents functional tests.
...
import httpx
import pytest
from hanzo_agents import Agent, AIConfig
...
def control_plane_url() -> str:
    """Get the Hanzo Agents control plane URL from environment."""
    url = os.environ.get("HANZO_AGENTS_SERVER", "http://localhost:8080")
    return url.rstrip("/")
...
def make_test_agent(control_plane_url: str) -> Callable[..., Agent]:
    ...
    def _factory(
        ...
        **kwargs
        ...
        kwargs.setdefault("hanzo_agents_server", control_plane_url)
...
async def registered_agent(
    ...
    try:
        await agent.hanzo_agents_handler.register_with_hanzo_agents_server(port)
