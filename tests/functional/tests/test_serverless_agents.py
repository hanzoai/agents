...
async def _register_serverless_via_cli(invocation_url: str):
    bin_override = os.environ.get("AF_BIN") or os.environ.get("HANZO_AGENTS_CLI")
    candidates = [bin_override] if bin_override else []
    candidates.extend(["af", "hanzo_agents"])
    ...
    env = os.environ.copy()
    env.setdefault("HANZO_AGENTS_SERVER", env.get("CONTROL_PLANE_URL", "http://localhost:8080"))
    token = env.get("HANZO_AGENTS_TOKEN")
...
async def run_python_serverless_agent(node_id: str, control_plane_url: str) -> AsyncIterator[str]:
    ...
    app = Agent(
        node_id=node_id,
        hanzo_agents_server=control_plane_url,
        auto_register=False,
        dev_mode=True,
    ...
    @app.reasoner()
    async def hello(name: str = "Hanzo Agents") -> dict:  # type: ignore[return-type]
        ctx = app.ctx
        return {
...
async def run_go_serverless_agent(node_id: str, control_plane_url: str) -> AsyncIterator[str]:
    ...
    env = {
        **os.environ,
        ...
        "HANZO_AGENTS_URL": control_plane_url,
        "PORT": str(port),
        "HANZO_AGENTS_TOKEN": os.environ.get("HANZO_AGENTS_TOKEN", ""),
    }
