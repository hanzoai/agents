...
async function loadSdk() {
  ...
  const candidates = [
    ...
    '/usr/local/lib/node_modules/@hanzo/agents-sdk/dist/index.js',
    '/usr/lib/node_modules/@hanzo/agents-sdk/dist/index.js'
  ].filter(Boolean);
  ...
  return await import('@hanzo/agents-sdk');
}

const { Agent } = await loadSdk();

const hanzoAgentsUrl = process.env.HANZO_AGENTS_SERVER ?? 'http://localhost:8080';
...
agent.reasoner('hello', async (ctx) => ({
  greeting: `Hello, ${ctx.input.name ?? 'Hanzo Agents'}!`,
  runId: ctx.runId,
