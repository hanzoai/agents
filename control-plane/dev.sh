...
#   ./dev.sh postgres  # Start with PostgreSQL (set HANZO_AGENTS_DATABASE_URL first)
#
# Prerequisites:
...
case "${1:-}" in
  postgres|pg)
    ...
    export HANZO_AGENTS_STORAGE_MODE=postgresql
    air -c .air.toml
    ;;
  *)
    echo "Starting control plane with SQLite (hot-reload)..."
    export HANZO_AGENTS_STORAGE_MODE=local
    air -c .air.toml
    ;;
esac
