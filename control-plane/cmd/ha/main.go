...
func runServer(cmd *cobra.Command, args []string) {
	logger.Logger.Debug().Msg("HanzoAgents server starting...")
	...
	fmt.Printf("HanzoAgents server running on http://localhost:%d\n", cfg.HanzoAgents.Port)
...
func loadConfig(configFile string) (*config.Config, error) {
	...
	if configFile != "" {
		...
		if envConfigFile := os.Getenv("HANZO_AGENTS_CONFIG_FILE"); envConfigFile != "" {
			...
			viper.AddConfigPath(".")
			viper.SetConfigName("hanzo-agents")
	...
	if cfg.UI.SourcePath == "" {
		candidateSourcePaths := []string{
			...
			filepath.Join(filepath.Dir(execDir), "apps", "platform", "hanzo-agents", "web", "client"),
			filepath.Join("apps", "platform", "hanzo-agents", "web", "client"),
	...
	if cfg.UI.DistPath == "" {
		candidateDistPaths := []string{
			...
			filepath.Join(filepath.Dir(execDir), "apps", "platform", "hanzo-agents", "web", "client", "dist"),
			filepath.Join("apps", "platform", "hanzo-agents", "web", "client", "dist"),
	...
	fmt.Printf("Loaded config - Storage mode: %s, HanzoAgents Port: %d, UI Mode: %s, UI Enabled: %t\n",
		cfg.Storage.Mode, cfg.HanzoAgents.Port, cfg.UI.Mode, cfg.UI.Enabled)
