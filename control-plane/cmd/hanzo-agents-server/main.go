...
var (
	...
	newHanzoAgentsServerFunc   = server.NewHanzoAgentsServer
	buildUIFunc               = buildUI
	...
	startHanzoAgentsServerFunc = defaultStartHanzoAgentsServer
)
...
func runServer(cmd *cobra.Command, args []string) {
	fmt.Println("HanzoAgents server starting...")
	...
	if cmd.Flags().Lookup("port").Changed {
		...
		cfg.HanzoAgents.Port = port
	}

	// Override from environment variables
	if envPort := os.Getenv("HANZO_AGENTS_PORT"); envPort != "" {
	...
	if !storageModeExplicit {
		if envMode := os.Getenv("HANZO_AGENTS_STORAGE_MODE"); envMode != "" {
	...
	if postgresURL == "" {
		if env := os.Getenv("HANZO_AGENTS_POSTGRES_URL"); env != "" {
			postgresURL = env
		} else if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_URL"); env != "" {
			postgresURL = env
		}
	}
	...
	if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_HOST"); env != "" {
		cfg.Storage.Postgres.Host = env
	}
	if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_PORT"); env != "" {
		if port, err := strconv.Atoi(env); err == nil {
			cfg.Storage.Postgres.Port = port
		}
	}
	if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_DATABASE"); env != "" {
		cfg.Storage.Postgres.Database = env
	}
	if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_USER"); env != "" {
		cfg.Storage.Postgres.User = env
	}
	if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_PASSWORD"); env != "" {
		cfg.Storage.Postgres.Password = env
	}
	if env := os.Getenv("HANZO_AGENTS_STORAGE_POSTGRES_SSLMODE"); env != "" {
	...
	hanzo-agentsServer, err := newHanzoAgentsServerFunc(cfg)
	if err != nil {
	...
	go func() {
		...
		if err := startHanzoAgentsServerFunc(hanzo-agentsServer); err != nil {
	...
	if cfg.UI.Enabled && openBrowserFlag && !backendOnly {
		uiTargetURL := fmt.Sprintf("http://localhost:%d", cfg.HanzoAgents.Port)
		if cfg.UI.Mode == "dev" {
	...
	fmt.Printf("HanzoAgents server running. Press Ctrl+C to exit.\n")
...
func loadConfig(configFile string) (*config.Config, error) {
	...
	viper.SetEnvPrefix("HANZO_AGENTS")
	...
	_ = viper.BindEnv("api.auth.api_key", "HANZO_AGENTS_API_KEY")
	_ = viper.BindEnv("api.auth.api_key", "HANZO_AGENTS_API_AUTH_API_KEY")
	...
	if configFile != "/dev/null" && configFile != "" {
		...
		if envConfigFile := os.Getenv("HANZO_AGENTS_CONFIG_FILE"); envConfigFile != "" {
			viper.SetConfigFile(envConfigFile)
		} else {
			viper.SetConfigName("hanzo-agents") // name of config file (without extension)
	...
	if cfg.HanzoAgents.Port == 0 {
		cfg.HanzoAgents.Port = 8080 // Default port
	}
	if cfg.Storage.Mode == "" {
	...
	if cfg.UI.SourcePath == "" {
		...
		if err != nil {
			cfg.UI.SourcePath = filepath.Join("apps", "platform", "hanzo-agents", "web", "client")
			...
			// This assumes the binary is built in the hanzo-agents/ directory
			cfg.UI.SourcePath = filepath.Join(execDir, "web", "client")

			// If that doesn't exist, try going up one level (if binary is in hanzo-agents/)
			if _, err := os.Stat(cfg.UI.SourcePath); os.IsNotExist(err) {
				cfg.UI.SourcePath = filepath.Join(filepath.Dir(execDir), "apps", "platform", "hanzo-agents", "web", "client")
			}

			// Final fallback to current working directory
			if _, err := os.Stat(cfg.UI.SourcePath); os.IsNotExist(err) {
				altPath := filepath.Join("apps", "platform", "hanzo-agents", "web", "client")
	...
	if cfg.Storage.Mode == "local" {
		...
		// Ensure all HanzoAgents data directories exist
		if _, err := utils.EnsureDataDirectories(); err != nil {
			return nil, fmt.Errorf("failed to create HanzoAgents data directories: %w", err)
		}
	}
...
func buildUI(cfg *config.Config) error {
	...
	buildEnv = append(buildEnv, fmt.Sprintf("VITE_API_PROXY_TARGET=http://localhost:%d", cfg.HanzoAgents.Port))
...
func defaultStartHanzoAgentsServer(s *server.HanzoAgentsServer) error {
	return s.Start()
}
