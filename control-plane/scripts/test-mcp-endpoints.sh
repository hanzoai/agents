...
# in the Hanzo Agents server. It includes comprehensive testing for all 4 endpoints
# with both success and error scenarios.
#
# Prerequisites:
# - Hanzo Agents server running on http://localhost:8080 (default)
...
#
# Usage:
#   chmod +x hanzo-agents/scripts/test-mcp-endpoints.sh
#   ./hanzo-agents/scripts/test-mcp-endpoints.sh
#
# =============================================================================
...
# Check if Hanzo Agents server is running
check_hanzo_agents_server() {
    log_info "Checking if Hanzo Agents server is running at $HANZO_AGENTS_SERVER..."

    if curl -s --connect-timeout 5 "$HANZO_AGENTS_SERVER/health" > /dev/null 2>&1; then
        log_success "Hanzo Agents server is running"
    else
...
main() {
    ...
    log_info "Hanzo Agents Server: $HANZO_AGENTS_SERVER"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -s, --server URL  Set Hanzo Agents server URL (default: http://localhost:8080)"
            echo "  -h, --help        Show this help message"
            echo ""
            echo "Environment variables:"
            echo "  HANZO_AGENTS_SERVER           Hanzo Agents server URL"
