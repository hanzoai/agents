...
# Hanzo Agents Release Automation Script
...
GITHUB_REPO="hanzoai/hanzo-agents"
GITHUB_OWNER="hanzoai"
GITHUB_REPO_NAME="hanzo-agents"
...
build_binaries() {
    ...
    local expected_binaries=(
        ...
        "hanzo-agents-linux-arm64"
        "hanzo-agents-darwin-amd64"
        "hanzo-agents-darwin-arm64"
    )
    ...
    ls -la "$DIST_DIR" | grep -E "(hanzo-agents-|checksums|build-info|README)"
}
...
# Hanzo Agents $NEXT_TAG (Pre-release)

This is a pre-release version of Hanzo Agents for testing purposes.
...
EOF
    ...
    if [ -n "$last_tag" ]; then
        ...
        echo "" >> "$release_notes_file"
        echo "- Initial release of Hanzo Agents Server" >> "$release_notes_file"
...
```bash
    curl -sSL https://raw.githubusercontent.com/hanzoai/hanzo-agents/main/scripts/install.sh | bash
```

### Manual Download
1. Download the appropriate binary for your platform from the assets below
2. Make it executable: `chmod +x hanzo-agents-*`
3. Run: `./hanzo-agents-linux-amd64` (or your platform's binary)
...
Report issues at: https://github.com/hanzoai/hanzo-agents/issues
...
main() {
    print_header "Hanzo Agents Server Release Automation"
...
case "${1:-}" in
    ...
    "build-only")
        ...
        ;;
    "help"|"-h"|"--help")
        echo "Hanzo Agents Release Automation Script"
        echo ""
        echo "Usage:"
