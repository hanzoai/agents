...
# Hanzo Agents Single Binary Builder
...
import (
 "fmt"
 "strings"
 "github.com/hanzoai/hanzo-agents/control-plane/internal/utils"
)

func main() {
 dirs, err := utils.GetHanzo AgentsDataDirectories()
 if err != nil {
  fmt.Printf("ERROR: %v\n", err)
  return
 }

 fmt.Printf("Hanzo Agents Home: %s\n", dirs.Hanzo AgentsHome)

 // Verify that Hanzo Agents Home points to ~/.hanzo-agents
 if !strings.HasSuffix(dirs.Hanzo AgentsHome, ".hanzo-agents") {
  fmt.Printf("ERROR: Hanzo Agents Home should end with .hanzo-agents, got: %s\n", dirs.Hanzo AgentsHome)
  return
 }
 ...
 if !strings.Contains(dbPath, ".hanzo-agents/data/hanzo-agents.db") {
...
build_all_binaries() {
    ...
    declare -a platforms=(
        # "linux:amd64:hanzo-agents-linux-amd64"
        # "linux:arm64:hanzo-agents-linux-arm64"
        # "darwin:amd64:hanzo-agents-darwin-amd64"
        "darwin:arm64:hanzo-agents-darwin-arm64"
    )
...
generate_metadata() {
    ...
    if command_exists sha256sum; then
        sha256sum hanzo-agents-* > checksums.txt 2>/dev/null || true
        print_success "Generated checksums.txt"
    elif command_exists shasum; then
        shasum -a 256 hanzo-agents-* > checksums.txt 2>/dev/null || true
        print_success "Generated checksums.txt"
    else
...
Hanzo Agents Single Binary Build Information
====================================
...
Usage:
  ./hanzo-agents-<platform>           # Start Hanzo Agents server with UI
  ./hanzo-agents-<platform> --help    # Show help
  ./hanzo-agents-<platform> --backend-only  # Start without UI

Data Storage:
All Hanzo Agents data is stored in ~/.hanzo-agents/ directory:
- ~/.hanzo-agents/data/hanzo-agents.db      # Main database
- ~/.hanzo-agents/data/hanzo-agents.bolt    # Cache/KV store
...
- HANZO_AGENTS_HOME: Override default ~/.hanzo-agents directory
- HANZO_AGENTS_PORT: Override default port (8080)
- HANZO_AGENTS_CONFIG_FILE: Override config file location
...
# Hanzo Agents Single Binary Distribution

This package contains pre-built Hanzo Agents binaries for multiple platforms.

## Quick Start

1. Download the appropriate binary for your platform:
   - `hanzo-agents-linux-amd64` - Linux (Intel/AMD 64-bit)
   - `hanzo-agents-linux-arm64` - Linux (ARM 64-bit)
   - `hanzo-agents-darwin-amd64` - macOS (Intel)
   - `hanzo-agents-darwin-arm64` - macOS (Apple Silicon)
   - `hanzo-agents-windows-amd64.exe` - Windows (64-bit)

2. Make the binary executable (Linux/macOS):
   ```bash
   chmod +x hanzo-agents-*
   ```

3. Run Hanzo Agents:
   ```bash
   ./hanzo-agents-linux-amd64
   ```
...
Hanzo Agents can be configured via:
- Environment variables (HANZO_AGENTS_HOME, HANZO_AGENTS_PORT, etc.)
- Configuration file (`~/.hanzo-agents/hanzo-agents.yaml`)
- Command line flags (`--port`, `--backend-only`, etc.)

## Data Directory

All Hanzo Agents data is stored in `~/.hanzo-agents/`:
```
~/.hanzo-agents/
├── data/
│   ├── hanzo-agents.db              # Main database
│   ├── hanzo-agents.bolt            # Cache
...
For issues and documentation, visit: https://github.com/hanzoai/hanzo-agents
EOF
...
show_summary() {
    ...
    if [ -d "$OUTPUT_DIR" ]; then
        ...
        ls -la "$OUTPUT_DIR" | grep -E "(hanzo-agents-|checksums|build-info|README)"
    ...
    echo "  cd $OUTPUT_DIR"
    echo "  ./hanzo-agents-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/amd64/')"
    echo ""
...
# Main build function
main() {
    print_header "Hanzo Agents Single Binary Builder"

    echo "Building Hanzo Agents single binary with:"
...
case "${1:-}" in
    ...
    "test-paths")
        ...
        ;;
    "help"|"-h"|"--help")
        echo "Hanzo Agents Single Binary Builder"
        echo ""
        echo "Usage:"
