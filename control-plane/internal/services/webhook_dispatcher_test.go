...
func TestWebhookDispatcher_DispatchWebhook_Success(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("X-HanzoAgents-Signature") != "" {
			require.NotEmpty(t, r.Header.Get("X-HanzoAgents-Signature"))
		}
...
func TestWebhookDispatcher_DispatchWebhook_WithSecret(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "POST", r.Method)
		signature := r.Header.Get("X-HanzoAgents-Signature")
...
func TestWebhookDispatcher_CustomHeaders(t *testing.T) {
	...
	if receivedHeaders != nil {
		...
		require.NotEmpty(t, receivedHeaders.Get("X-HanzoAgents-Signature"))
	}
}
