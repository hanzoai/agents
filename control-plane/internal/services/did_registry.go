...
func (r *DIDRegistry) StoreRegistry(registry *types.DIDRegistry) error {
	...
	// Store in memory
	r.registries[registry.HanzoAgentsServerID] = registry
...
func (r *DIDRegistry) saveRegistryToDatabase(registry *types.DIDRegistry) error {
	...
	err := r.storageProvider.StoreHanzoAgentsServerDID(
		ctx,
		registry.HanzoAgentsServerID,
		registry.RootDID,
		registry.MasterSeed,
		registry.CreatedAt,
	...
	for _, agentInfo := range registry.AgentNodes {
		...
		err := r.storageProvider.StoreAgentDIDWithComponents(
			...
			agentInfo.DID,
			registry.HanzoAgentsServerID, // Use af server ID instead of root DID
