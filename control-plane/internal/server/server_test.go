...
func TestCheckCacheHealthHealthy(t *testing.T) {
	cache := newFakeCache()
	srv := &HanzoAgentsServer{cache: cache}
...
func TestGenerateHanzoAgentsServerIDDeterministic(t *testing.T) {
	dir1 := filepath.Join("/tmp", "hanzo-agents-test-1")
	dir2 := filepath.Join("/tmp", "hanzo-agents-test-2")

	id1 := generateHanzoAgentsServerID(dir1)
	id1Again := generateHanzoAgentsServerID(dir1)
	id2 := generateHanzoAgentsServerID(dir2)
...
func TestSyncPackagesFromRegistry(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(pkgDir, "hanzo-agents-package.yaml"), packageContent, 0o644); err != nil {
		t.Fatalf("failed to write hanzo-agents-package.yaml: %v", err)
	}
