...
func TestStreamExecutionEventsHandler(t *testing.T) {
	...
	router := gin.New()
	router.GET("/v1/ui/executions/events", handler.StreamExecutionEventsHandler)

	req := httptest.NewRequest(http.MethodGet, "/v1/ui/executions/events", nil)
...
func TestSSERequestValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"POST", "/v1/ui/executions/events"},
		{"PUT", "/v1/ui/executions/events"},
	}
...
func TestSSEWithQueryParameters(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/ui/executions/events?filter=test&limit=10", nil)
	resp := httptest.NewRecorder()
