...
func TestGetNodesSummaryHandler_Structure(t *testing.T) {
	...
	router := gin.New()
	router.GET("/v1/ui/nodes", handler.GetNodesSummaryHandler)
...
func TestGetNodeStatusHandler_Structure(t *testing.T) {
	...
	handler := NewNodesHandler(uiService)
	router := gin.New()
	router.GET("/v1/ui/nodes/:nodeId/status", handler.GetNodeStatusHandler)

	req := httptest.NewRequest(http.MethodGet, "/v1/ui/nodes/node-1/status", nil)
	resp := httptest.NewRecorder()
...
func TestRefreshNodeStatusHandler_Structure(t *testing.T) {
	...
	handler := NewNodesHandler(uiService)
	router := gin.New()
	router.POST("/v1/ui/nodes/:nodeId/status/refresh", handler.RefreshNodeStatusHandler)

	req := httptest.NewRequest(http.MethodPost, "/v1/ui/nodes/node-1/status/refresh", nil)
	resp := httptest.NewRecorder()
...
func TestBulkNodeStatusHandler_Validation(t *testing.T) {
	...
	handler := NewNodesHandler(uiService)
	router := gin.New()
	router.POST("/v1/ui/nodes/status/bulk", handler.BulkNodeStatusHandler)

	// Test with invalid JSON
	req := httptest.NewRequest(http.MethodPost, "/v1/ui/nodes/status/bulk", strings.NewReader("invalid json"))
	...
	req = httptest.NewRequest(http.MethodPost, "/v1/ui/nodes/status/bulk", strings.NewReader(`{}`))
	req.Header.Set("Content-Type", "application/json")
	resp = httptest.NewRecorder()
	...
	req = httptest.NewRequest(http.MethodPost, "/v1/ui/nodes/status/bulk", strings.NewReader(`{"node_ids": ["node-1", "node-2"]}`))
	req.Header.Set("Content-Type", "application/json")
	resp = httptest.NewRecorder()
...
func TestGetDashboardSummaryHandler_Structure(t *testing.T) {
	...
	router := gin.New()
	router.GET("/v1/ui/dashboard", handler.GetDashboardSummaryHandler)

	req := httptest.NewRequest(http.MethodGet, "/v1/ui/dashboard", nil)
	resp := httptest.NewRecorder()
...
func TestAPIErrorHandling(t *testing.T) {
	...
	tests := []struct {
		...
		{"empty nodeId", "/v1/ui/nodes/", "GET"},
		{"special chars in nodeId", "/v1/ui/nodes/node%20with%20spaces", "GET"},
		{"very long nodeId", "/v1/ui/nodes/" + strings.Repeat("a", 1000), "GET"},
	}
...
func TestAPIMethodValidation(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/ui/nodes", nil)
	resp := httptest.NewRecorder()
	router.ServeHTTP(resp, req)
	assert.Equal(t, http.StatusNotFound, resp.Code) // Gin returns 404 for wrong method

	// Test POST endpoint with wrong method
	req = httptest.NewRequest(http.MethodGet, "/v1/ui/nodes/node-1/status/refresh", nil)
	resp = httptest.NewRecorder()
	router.ServeHTTP(resp, req)
