...
func TestExecuteAsyncHandler_QueueSaturation(t *testing.T) {
	...
	originalCapacity := os.Getenv("HANZO_AGENTS_EXEC_ASYNC_QUEUE_CAPACITY")
	defer func() {
		if originalCapacity != "" {
			os.Setenv("HANZO_AGENTS_EXEC_ASYNC_QUEUE_CAPACITY", originalCapacity)
		} else {
			os.Unsetenv("HANZO_AGENTS_EXEC_ASYNC_QUEUE_CAPACITY")
		}
	}()

	// Set a very small capacity to make saturation easier to test
	os.Setenv("HANZO_AGENTS_EXEC_ASYNC_QUEUE_CAPACITY", "2")
