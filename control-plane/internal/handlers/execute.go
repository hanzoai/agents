...
func getAsyncWorkerPool() *asyncWorkerPool {
	asyncPoolOnce.Do(func() {
		workerCount := resolveIntFromEnv("HANZO_AGENTS_EXEC_ASYNC_WORKERS", runtime.NumCPU())
		if workerCount <= 0 {
			workerCount = runtime.NumCPU()
		}

		queueCapacity := resolveIntFromEnv("HANZO_AGENTS_EXEC_ASYNC_QUEUE_CAPACITY", 1024)
		if queueCapacity <= 0 {
			queueCapacity = 1024
		}
...
func ensureCompletionWorker() {
	completionOnce.Do(func() {
		size := resolveIntFromEnv("HANZO_AGENTS_EXEC_COMPLETION_QUEUE", 2048)
		if size <= 0 {
			size = 2048
		}
