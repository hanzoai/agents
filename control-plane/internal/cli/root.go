...
// NewRootCommand creates and returns the root Cobra command for the HanzoAgents CLI.
func NewRootCommand(runServerFunc func(cmd *cobra.Command, args []string), versionInfo VersionInfo) *cobra.Command {
	RootCmd := &cobra.Command{
		Use:     "af",
		Aliases: []string{"hanzo-agents"},
		Short:   "HanzoAgents AI Agent Platform",
		Long:    `HanzoAgents is a comprehensive AI agent platform for building, managing, and deploying AI agent capabilities.`,
	...
	RootCmd.Run = func(cmd *cobra.Command, args []string) {
		if showVersion {
			fmt.Printf("HanzoAgents Control Plane\n")
	...
	RootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "Path to configuration file (e.g., config/hanzo-agents.yaml)")
	...
	services := application.CreateServiceContainer(cfg, getHanzoAgentsHomeDir())
	...
	serverCmd := &cobra.Command{
		Use:   "server",
		Short: "Run the HanzoAgents AI Agent Platform server",
		Long:  `Starts the HanzoAgents AI Agent Platform server, providing API endpoints and UI.`,
		Run:   runServerFunc,
	}
