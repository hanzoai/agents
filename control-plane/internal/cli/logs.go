...
func NewLogsCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "View logs for a HanzoAgents agent node",
		Long: `Display logs for an installed HanzoAgents agent node package.
...
func (lv *LogViewer) ViewLogs(agentNodeName string) error {
	...
	registryPath := filepath.Join(lv.HanzoAgentsHome, "installed.yaml")
