...
func NewConfigCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Configure environment variables for an installed HanzoAgents agent package",
		Long: `Configure environment variables for an installed HanzoAgents agent package.
...
func (pcm *PackageConfigManager) loadPackageMetadata(packageName string) (*packages.PackageMetadata, string, error) {
	...
	registryPath := filepath.Join(pcm.HanzoAgentsHome, "installed.yaml")
	...
	// Load package metadata
	metadataPath := filepath.Join(installedPackage.Path, "hanzo-agents-package.yaml")
...
func (pcm *PackageConfigManager) saveEnvFile(packagePath string, envVars map[string]string) error {
	...
	var lines []string
	lines = append(lines, "# Environment variables for HanzoAgents agent")
