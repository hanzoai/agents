...
func (cmd *DevCommand) GetDescription() string {
	return "Run a HanzoAgents agent package in development mode"
}
...
func (cmd *DevCommand) BuildCobraCommand() *cobra.Command {
	...
	cobraCmd := &cobra.Command{
		...
		Long: `Run a HanzoAgents agent package in development mode from the current directory or specified path.

This command is designed for local development and testing. It will:
- Look for hanzo-agents.yaml in the current directory (or specified path)
...
func (cmd *DevCommand) execute(args []string, port int, watch, verbose bool) error {
	...
	cmd.output.PrintHeader("🚀 HanzoAgents Development Mode")
