...
func (cmd *InstallCommand) GetDescription() string {
	return "Install a HanzoAgents agent node package for local use"
}
...
func (cmd *InstallCommand) BuildCobraCommand() *cobra.Command {
	...
	cobraCmd := &cobra.Command{
		...
		Long: `Install a HanzoAgents agent node package for local use.

The package can be:
- A local directory path
- A GitHub repository URL
- A package name from the HanzoAgents registry

Examples:
  hanzo-agents install ./my-agent
  hanzo-agents install https://github.com/user/agent-repo
  hanzo-agents install agent-name`,
...
func (cmd *InstallCommand) execute(packagePath string, force, verbose bool) error {
	cmd.output.PrintHeader("Installing HanzoAgents Package")
