...
func NewListCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List installed HanzoAgents agent node packages",
		Long: `Display all installed HanzoAgents agent node packages with their status.
...
func runListCommand(cmd *cobra.Command, args []string) {
	...
	if len(registry.Installed) == 0 {
		...
		fmt.Println("💡 Install packages with: hanzo-agents install <package-path>")
		return
	}
