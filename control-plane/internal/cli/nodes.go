...
func newRegisterServerlessCommand() *cobra.Command {
	opts := &registerServerlessOptions{
		serverURL: os.Getenv("HANZO_AGENTS_SERVER"),
	...
	cmd.Flags().StringVar(&opts.serverURL, "server", opts.serverURL, "Control plane URL (default: http://localhost:8080 or $HANZO_AGENTS_SERVER)")
	cmd.Flags().StringVar(&opts.token, "token", opts.token, "Bearer token for the control plane (default: $HANZO_AGENTS_TOKEN)")
