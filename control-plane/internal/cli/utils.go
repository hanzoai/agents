...
// getHanzoAgentsHomeDir returns the HanzoAgents home directory (~/.hanzo-agents) and ensures it exists
func getHanzoAgentsHomeDir() string {
	if customHome := os.Getenv("HANZO_AGENTS_HOME"); customHome != "" {
		if err := os.MkdirAll(customHome, 0755); err != nil {
			PrintError(fmt.Sprintf("Failed to create HANZO_AGENTS_HOME directory: %v", err))
			os.Exit(1)
		}
