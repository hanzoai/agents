...
func (m initModel) View() string {
	...
	// Title
	s.WriteString(titleStyle.Render("🎯 Creating HanzoAgents Agent") + "\n\n")
...
func NewInitCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Initialize a new HanzoAgents agent project",
		Long: `Initialize a new HanzoAgents agent project with a predefined
...
- Configuration for connecting to the HanzoAgents control plane
...
Example:
  ...
  af init my-agent -l typescript`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("  3. af server                    # Start HanzoAgents server")
			...
			fmt.Println()
			printInfo("Learn more: https://hanzo-agents.ai/docs")
			fmt.Println()
