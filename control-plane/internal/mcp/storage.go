...
// It stores all MCP server configurations in a single hanzo-agents.yaml file
...
// projectDir is the root directory of the hanzo-agents project.
func NewYAMLConfigStorage(projectDir string) *YAMLConfigStorage {
	return &YAMLConfigStorage{
		ProjectDir: projectDir,
		filePath:   filepath.Join(projectDir, "hanzo-agents.yaml"),
	}
}
...
type hanzo-agentsYAML struct {
	...
	// Other fields in hanzo-agents.yaml are preserved but not directly managed here.
...
func (s *YAMLConfigStorage) loadHanzoAgentsYAML() (*hanzo-agentsYAML, error) {
	...
	if err := yaml.Unmarshal(data, &cfg); err != nil {
		return nil, fmt.Errorf("failed to unmarshal hanzo-agents.yaml: %w", err)
	}
	return &cfg, nil
}
...
func (s *YAMLConfigStorage) SaveMCPServerConfig(alias string, config *MCPServerConfig) error {
	...
	return s.saveHanzoAgentsYAML(cfg)
}
