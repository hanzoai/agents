...
func (sg *SkillGenerator) GenerateSkillsForServer(serverAlias string) (*SkillGenerationResult, error) {
	...
	cfg, err := config.LoadConfig(filepath.Join(sg.projectDir, "hanzo-agents.yaml"))
	if err != nil {
		// Fallback to current directory
		cfg, err = config.LoadConfig("hanzo-agents.yaml")
		if err != nil {
...
func (sg *SkillGenerator) generateDocString(tool MCPTool, parameters []SkillParameter) string {
	...
	docString.WriteString(`
        execution_context (ExecutionContext, optional): HanzoAgents execution context for workflow tracking
...
from hanzo-agents import app
from hanzo-agents.execution_context import ExecutionContext
from hanzo-agents.mcp.client import MCPClient
from hanzo-agents.mcp.exceptions import (
    MCPError, MCPConnectionError, MCPToolError, MCPTimeoutError
)
from hanzo-agents.agent import Agent
