...
func (m *MCPManager) Add(config MCPServerConfig) error {
	...
	// Update hanzo-agents.yaml
	if err := m.updateHanzoAgentsYAML(config); err != nil {
		return fmt.Errorf("failed to update hanzo-agents.yaml: %w", err)
	}
...
func (m *MCPManager) Remove(alias string) error {
	...
	// Update hanzo-agents.yaml
	if err := m.removeMCPFromHanzoAgentsYAML(alias); err != nil {
		return fmt.Errorf("failed to update hanzo-agents.yaml: %w", err)
	}
...
func (m *MCPManager) updateHanzoAgentsYAML(config MCPServerConfig) error {
	...
	if err := yaml.Unmarshal(data, &yamlConfig); err != nil {
		return fmt.Errorf("failed to parse hanzo-agents.yaml: %w", err)
	}
...
// loadMCPConfigsFromYAML loads MCP configurations from hanzo-agents.yaml
//
...
func (m *MCPManager) loadMCPConfigsFromYAML() (map[string]MCPServerConfig, error) {
	...
	var config map[string]interface{}
	if err := yaml.Unmarshal(data, &config); err != nil {
		return nil, fmt.Errorf("failed to parse hanzo-agents.yaml: %w", err)
	}
...
func (m *MCPManager) DiscoverCapabilities(alias string) (*MCPManifest, error) {
	...
	if m.verbose {
		...
		fmt.Printf("Note: MCP skills will be auto-registered by HanzoAgents SDK when agent starts\n")
	}
