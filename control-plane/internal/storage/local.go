...
func (ls *LocalStorage) getWorkflowExecutionByID(ctx context.Context, q DBTX, executionID string) (*types.WorkflowExecution, error) {
	query := `
		SELECT workflow_id, execution_id, hanzo_agents_request_id, run_id, session_id, actor_id,
	...
	err := row.Scan(
		&execution.WorkflowID, &execution.ExecutionID, &execution.HanzoAgentsRequestID,
...
func (ls *LocalStorage) initializeSQLite(ctx context.Context) error {
	...
	busyTimeout := resolveEnvInt("HANZO_AGENTS_SQLITE_BUSY_TIMEOUT_MS", 60000)
	if busyTimeout <= 0 {
		busyTimeout = 60000
	}
	...
	idleConns := resolveEnvInt("HANZO_AGENTS_SQLITE_MAX_IDLE_CONNS", 1)
	if idleConns < 0 {
		idleConns = 0
	}
...
func (ls *LocalStorage) ensureSQLiteIndexes() error {
	indexStatements := []string{
		...
		"CREATE INDEX IF NOT EXISTS idx_agent_dids_hanzo_agents_server ON agent_dids(hanzo_agents_server_id)",
...
const sqliteWorkflowExecutionInsertQuery = `INSERT INTO workflow_executions (
	workflow_id, execution_id, hanzo_agents_request_id, run_id, session_id, actor_id,
	agent_node_id, parent_workflow_id, parent_execution_id, root_workflow_id, workflow_depth,
	reasoner_id, input_data, output_data, input_size, output_size,
	status, started_at, completed_at, duration_ms,
	state_version, last_event_sequence, active_children, pending_children,
...
func (ls *LocalStorage) executeWorkflowInsert(ctx context.Context, q DBTX, execution *types.WorkflowExecution) error {
	...
	_, err = q.ExecContext(ctx, insertQuery,
		execution.WorkflowID, execution.ExecutionID, execution.HanzoAgentsRequestID, execution.RunID,
...
func (ls *LocalStorage) QueryWorkflowExecutions(ctx context.Context, filters types.WorkflowExecutionFilters) ([]*types.WorkflowExecution, error) {
	...
	baseQuery := `
		SELECT
			...
			workflow_executions.hanzo_agents_request_id, workflow_executions.run_id, workflow_executions.session_id, workflow_executions.actor_id,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&execution.HanzoAgentsRequestID, &runID, &execution.SessionID, &execution.ActorID,
...
func (ls *LocalStorage) QueryWorkflowDAG(ctx context.Context, rootWorkflowID string) ([]*types.WorkflowExecution, error) {
	...
	query := `
		WITH RECURSIVE workflow_dag AS (
			...
			SELECT
				id, workflow_id, execution_id, hanzo_agents_request_id, run_id, session_id, actor_id,
			...
			SELECT
				we.id, we.workflow_id, we.execution_id, we.hanzo_agents_request_id, we.run_id, we.session_id, we.actor_id,
...
func (ls *LocalStorage) StoreHanzoAgentsServerDID(ctx context.Context, hanzoAgentsServerID, rootDID string, masterSeed []byte, createdAt, lastKeyRotation time.Time) error {
	...
	defer func() {
		if err != nil {
			rollbackTx(tx, "StoreHanzoAgentsServerDID")
		}
	}()
	...
	err = ls.retryOnConstraintFailure(ctx, func() error {
		query := `
                        INSERT OR REPLACE INTO did_registry (hanzo_agents_server_id, root_did, master_seed_encrypted, created_at, last_key_rotation, total_dids)
                        VALUES (?, ?, ?, ?, ?, 0)
                `
		if ls.mode == "postgres" {
			query = `
                                INSERT INTO did_registry (hanzo_agents_server_id, root_did, master_seed_encrypted, created_at, last_key_rotation, total_dids)
                                VALUES (?, ?, ?, ?, ?, 0)
                                ON CONFLICT (hanzo_agents_server_id) DO UPDATE SET
	...
	err = ls.retryOnConstraintFailure(ctx, func() error {
		query := `
			INSERT INTO agent_dids (
				agent_node_id, did, hanzo_agents_server_id, public_key_jwk, derivation_path, registered_at, status
			) VALUES (?, ?, ?, ?, ?, ?, ?)`
...
func (ls *LocalStorage) ListHanzoAgentsServerDIDs(ctx context.Context) ([]*types.HanzoAgentsServerDIDInfo, error) {
	// Check context cancellation early
	if err := ctx.Err(); err != nil {
	...
	var infos []*types.HanzoAgentsServerDIDInfo
	for rows.Next() {
		// Check context cancellation during iteration
		if err := ctx.Err(); err != nil {
		...
		info := &types.HanzoAgentsServerDIDInfo{}
		err := rows.Scan(&info.HanzoAgentsServerID, &info.RootDID, &info.MasterSeed, &info.CreatedAt, &info.LastKeyRotation)
		if err != nil {
...
func (ls *LocalStorage) GetAgentDID(ctx context.Context, agentID string) (*types.AgentDIDInfo, error) {
	...
	query := `
		SELECT agent_node_id, did, hanzo_agents_server_id, public_key_jwk, derivation_path,
	...
	err := row.Scan(&info.AgentNodeID, &info.DID, &info.HanzoAgentsServerID, &publicKeyJWK,
...
func (ls *LocalStorage) ListAgentDIDs(ctx context.Context) ([]*types.AgentDIDInfo, error) {
	...
	for rows.Next() {
		...
		info := &types.AgentDIDInfo{}
		var reasonersJSON, skillsJSON, publicKeyJWK string
		err := rows.Scan(&info.AgentNodeID, &info.DID, &info.HanzoAgentsServerID, &publicKeyJWK,
			&info.DerivationPath, &reasonersJSON, &skillsJSON, &info.Status, &info.RegisteredAt)
		if err != nil {
