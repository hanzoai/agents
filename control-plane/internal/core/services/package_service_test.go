...
func TestInstallPackage_GitURL(t *testing.T) {
	...
	assert.NotContains(t, err.Error(), "hanzo-agents-package.yaml")
}
...
`
	...
	// The error should not be about missing hanzo-agents-package.yaml or main.py
	if err != nil {
		assert.NotContains(t, err.Error(), "hanzo-agents-package.yaml not found")
...
func TestInstallLocalPackage_MissingPackageYaml(t *testing.T) {
	...
	err := service.InstallPackage(sourcePath, options)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "hanzo-agents-package.yaml")
}
...
func TestValidatePackage_Success(t *testing.T) {
	...
	packageYamlPath := filepath.Join(packagePath, "hanzo-agents-package.yaml")
