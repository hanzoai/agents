...
func TestRunInDevMode_NoHanzoAgentsYaml(t *testing.T) {
	tmpDir := t.TempDir()
	...
	// Mock file system to report hanzo-agents.yaml doesn't exist
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "no hanzo-agents.yaml found")
}

func TestRunInDevMode_HanzoAgentsYamlExists(t *testing.T) {
	tmpDir := t.TempDir()
	packagePath := filepath.Join(tmpDir, "test-package")
	require.NoError(t, os.MkdirAll(packagePath, 0755))
	...
	// But we verify it gets past the hanzo-agents.yaml check
	err := service.RunInDevMode(packagePath, options)
	// The error should be about process startup or port discovery, not about hanzo-agents.yaml
	if err != nil {
		assert.NotContains(t, err.Error(), "no hanzo-agents.yaml found")
	}
}
