...
func TestLoadHanzoAgentsPackageConfig(t *testing.T) {
	// Create a temporary test directory
	tempDir := t.TempDir()

	// Create a test hanzo-agents-package.yaml
	testConfig := `
name: "test-agent"
version: "1.0.0"
...
metadata:
  ...
  sdk_version: "1.0.0"
  language: "python"
  platform: "hanzo-agents-agent-node"
`

	configPath := filepath.Join(tempDir, "hanzo-agents-package.yaml")
	...
	config, err := LoadHanzoAgentsPackageConfig(tempDir)
	if err != nil {
