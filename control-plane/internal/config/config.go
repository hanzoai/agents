...
type Config struct {
	HanzoAgents HanzoAgentsConfig `yaml:"hanzo-agents" mapstructure:"hanzo-agents"`
...
// HanzoAgentsConfig holds the core HanzoAgents server configuration.
type HanzoAgentsConfig struct {
	Port             int                    `yaml:"port"`
...
const DefaultConfigPath = "hanzo-agents.yaml" // Or "./hanzo-agents.yaml", "config/hanzo-agents.yaml" depending on convention
...
func LoadConfig(configPath string) (*Config, error) {
	...
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		...
		altPath := filepath.Join("config", "hanzo-agents.yaml")
...
func applyEnvOverrides(cfg *Config) {
	// API Authentication
	if apiKey := os.Getenv("HANZO_AGENTS_API_KEY"); apiKey != "" {
	...
	if val := os.Getenv("HANZO_AGENTS_HEALTH_CHECK_INTERVAL"); val != "" {
		if d, err := time.ParseDuration(val); err == nil {
			cfg.HanzoAgents.NodeHealth.CheckInterval = d
		}
	}
	if val := os.Getenv("HANZO_AGENTS_HEALTH_CHECK_TIMEOUT"); val != "" {
		if d, err := time.ParseDuration(val); err == nil {
			cfg.HanzoAgents.NodeHealth.CheckTimeout = d
		}
	}
	if val := os.Getenv("HANZO_AGENTS_HEALTH_CONSECUTIVE_FAILURES"); val != "" {
		if i, err := strconv.Atoi(val); err == nil {
			cfg.HanzoAgents.NodeHealth.ConsecutiveFailures = i
		}
	}
	if val := os.Getenv("HANZO_AGENTS_HEALTH_RECOVERY_DEBOUNCE"); val != "" {
		if d, err := time.ParseDuration(val); err == nil {
			cfg.HanzoAgents.NodeHealth.RecoveryDebounce = d
		}
	}
	if val := os.Getenv("HANZO_AGENTS_HEARTBEAT_STALE_THRESHOLD"); val != "" {
		if d, err := time.ParseDuration(val); err == nil {
