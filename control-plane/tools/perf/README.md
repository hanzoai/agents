...
1. Build the gateway binary (or run `go run ./cmd/hanzo-agents-server`). The production
   ...
   ```bash
   cd hanzo-agents/apps/platform/hanzo-agents
   ./build-single-binary.sh
   ./dist/releases/hanzo-agents-darwin-arm64 --config ./configs/local.yaml
   ```
...
2. Install the load-test dependencies in a virtual environment (or build the
   ...
   ```bash
   cd hanzo-agents/apps/platform/hanzo-agents/tools/perf
   python -m venv .venv
...
```bash
docker build -t hanzo-agents-perf ./apps/platform/hanzo-agents/tools/perf
```
...
docker run --rm --network host hanzo-agents-perf \
...
To exercise the harness against a containerised Hanzo Agents server backed by the new
SQLite + GORM storage layer, reuse the Docker image from
`apps/platform/hanzo-agents/docker`:

```bash
# 1) Build and run the Hanzo Agents server (see docker/README.md for details)
docker build -t hanzo-agents-local apps/platform/hanzo-agents/docker
docker run --rm -d --name hanzo-agents-local \
  -p 8080:8080 \
  -v "$(pwd)/apps/platform/hanzo-agents/data:/app/data" \
  hanzo-agents-local
...
# 3) Stop the Hanzo Agents container when finished
docker stop hanzo-agents-local
```
...
- **Backpressure verification**: Drive queue overload by setting
  ...
  (`hanzo_agents_gateway_queue_depth`, `hanzo_agents_gateway_backpressure_total`) move as
  expected.
