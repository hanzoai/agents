...
Hanzo Agents SDKs shown in blue family (visually grouped), external frameworks in distinct colors.
"""
...
# Color palette: Hanzo Agents SDKs in blue gradient, external frameworks in distinct colors
COLORS = {
    # Hanzo Agents SDKs - Blue family (visually grouped)
    "Hanzo Agents_Go": "#0D2137",         # Deep navy
    "Hanzo Agents_TypeScript": "#1A5276", # Medium blue
    "Hanzo Agents_Python": "#3498DB",     # Bright blue
    # External frameworks
...
LABELS = {
    "Hanzo Agents_Go": "Hanzo Agents (Go)",
    "Hanzo Agents_TypeScript": "Hanzo Agents (TS)",
    "Hanzo Agents_Python": "Hanzo Agents (Python)",
...
def load_results(results_dir: Path) -> dict:
    ...
    for f in results_dir.glob("*.json"):
        if f.name.startswith(("Hanzo Agents", "LangChain", "CrewAI", "Mastra")):
            with open(f) as fp:
                data = json.load(fp)
...
def create_benchmark_figure(results: dict, output_dir: Path):
    ...
    # Framework order: Hanzo Agents SDKs first (grouped), then external
    frameworks = [
        "Hanzo Agents_Go",
        "Hanzo Agents_TypeScript",
        "Hanzo Agents_Python",
        "LangChain_Python",
        "CrewAI_Python",
        "Mastra_TypeScript",
