...
echo "=============================================="
echo "Hanzo Agents Scale Benchmark Suite"
echo "=============================================="
echo ""
...
./benchmark --handlers=100000 --iterations=10 --warmup=2 --json > "$RESULTS_DIR/Hanzo Agents_Go.json"

echo "Go benchmark complete."
cat "$RESULTS_DIR/Hanzo Agents_Go.json" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f\"  Registration: {[r['value'] for r in d['results'] if r['metric']=='registration_time_mean_ms'][0]:.2f}ms\")"
...
python3 benchmark.py --handlers=5000 --iterations=10 --warmup=2 --json > "$RESULTS_DIR/Hanzo Agents_Python.json"

echo "Python benchmark complete."
cat "$RESULTS_DIR/Hanzo Agents_Python.json" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f\"  Registration: {[r['value'] for r in d['results'] if r['metric']=='registration_time_mean_ms'][0]:.2f}ms\")"
