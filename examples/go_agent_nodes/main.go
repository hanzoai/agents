...
func main() {
	...
	controlPlaneURL := strings.TrimSpace(os.Getenv("HANZO_AGENTS_URL"))
	...
	cfg := agent.Config{
		NodeID:        nodeID,
		Version:       "1.0.0",
		HanzoAgentsURL: controlPlaneURL, // optional for CLI-only
		Token:         os.Getenv("HANZO_AGENTS_TOKEN"),
		...
		CLIConfig: &agent.CLIConfig{
			...
			EnvironmentVars: []string{
				"HANZO_AGENTS_URL (optional) Control plane URL for server mode",
				"HANZO_AGENTS_TOKEN (optional) Bearer token",
	...
	hasControlPlane := strings.TrimSpace(cfg.HanzoAgentsURL) != ""
