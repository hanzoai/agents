...
from hanzo_agents import AgentRouter
from hanzo_agents.logger import log_info
...
"Hanzo Agents uses DIDs for identity [A]. The control plane manages orchestration [B]. You can deploy agents independently [A][B]."
...
**Good Answer:**
To get started with Hanzo Agents:

1. Install the CLI: `npm install -g hanzo_agents` [A]
...
**Good Answer:**
Hanzo Agents uses Decentralized Identifiers (DIDs) for identity management [A]. Each agent receives a unique, cryptographically verifiable DID when registered [A]. You can configure IAM policies in the control plane settings under `config/hanzo_agents.yaml` in the `security` section [B].
