...
This chunks every `.md`, `.mdx`, `.rst`, or `.txt` file, embeds them, and stores vectors in Hanzo Agents's global memory scope using the two-tier storage strategy.
...
|----------|-------------|---------|
| `HANZO_AGENTS_SERVER` | Control plane server URL | `http://localhost:8080` |
| `DOC_EMBED_MODEL` | FastEmbed model for embeddings | `BAAI/bge-small-en-v1.5` |
| `AI_MODEL` | Primary LLM (handled by Hanzo Agents `AIConfig`) | `openrouter/openai/gpt-4o-mini` |
...
- Stream answers token-by-token via Hanzo Agents streaming APIs
