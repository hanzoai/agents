...
Minimal Retrieval-Augmented agent that demonstrates the new Hanzo Agents memory vector APIs without any external services.
...
Once the agent is registered with the control plane, you can call it via the Hanzo Agents CLI or direct HTTP:

```bash
# Ingest a document (either provide text or a path)
hanzo-agents call hello-world-rag ingest_document \
  --json '{"document_id": "guide", "path": "docs/README.md"}'

# Ask a question
hanzo-agents call hello-world-rag answer_question \
