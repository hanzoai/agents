...
This document describes how to create releases for Hanzo Agents using the **two-tier release system**.

## Overview

Hanzo Agents uses a two-tier release model that separates **staging** (prerelease) from **production** releases:
...
- Python: PyPI as prerelease (`pip install --pre hanzo-agents`)
- TypeScript: npm with `@next` tag
- Docker: `hanzo-agents/control-plane:staging-X.Y.Z-rc.N`
...
1. Go to Actions -> [Release workflow](https://github.com/hanzoai/agents/actions/workflows/release.yml)
...
curl -fsSL https://hanzo-agents.ai/install.sh | bash -s -- --staging

# Or directly from GitHub
curl -fsSL https://raw.githubusercontent.com/hanzoai/agents/main/scripts/install.sh | bash -s -- --staging

# Python (prerelease - requires --pre flag)
pip install --pre hanzo-agents

# TypeScript
npm install @hanzo-agents/sdk@next

# Docker
docker pull hanzo-agents/control-plane:staging-0.1.28-rc.4
```
...
2. Go to Actions -> [Release workflow](https://github.com/hanzoai/agents/actions/workflows/release.yml)
...
- Docker: `hanzo-agents/control-plane:vX.Y.Z` + `:latest`
...
```bash
# Binary (recommended)
curl -fsSL https://hanzo-agents.ai/install.sh | bash
...
# Docker
docker pull hanzo-agents/control-plane:latest
```
...
```
hanzo-agents-darwin-amd64          # macOS Intel binary
hanzo-agents-darwin-arm64          # macOS Apple Silicon binary
hanzo-agents-linux-amd64           # Linux x86_64 binary
hanzo-agents-linux-arm64           # Linux ARM64 binary
checksums.txt                    # SHA256 checksums for all binaries
hanzo-agents-X.Y.Z-py3-none-any.whl   # Python wheel
hanzo-agents-X.Y.Z.tar.gz             # Python source distribution
```
...
|----------|---------|------------|
| PyPI | `pip install --pre hanzo-agents` | `pip install hanzo-agents` |
| npm | `@hanzo-agents/sdk@next` | `@hanzo-agents/sdk@latest` |
| Docker | `hanzo-agents/control-plane:staging-*` | `hanzo-agents/control-plane:v*` |
...
# Specific version
VERSION=v0.1.28 curl -fsSL https://hanzo-agents.ai/install.sh | bash
```
...
STAGING=1 curl -fsSL https://hanzo-agents.ai/install.sh | bash

# Specific prerelease version
VERSION=v0.1.28-rc.4 curl -fsSL https://hanzo-agents.ai/install.sh | bash -s -- --staging
```
...
1. Check the [Actions tab](https://github.com/hanzoai/agents/actions/workflows/release.yml) for the automatic run
2. Verify:
   - [ ] Workflow triggered automatically
   - [ ] Version bumped to `X.Y.Z-rc.N` (e.g., `0.1.19-rc.1`)
   - [ ] Python prerelease package appears on PyPI
   - [ ] `npm install @hanzo-agents/sdk@next` installs new version
   - [ ] GitHub release marked as "Pre-release"
   - [ ] Docker image tagged `staging-X.Y.Z-rc.N`
3. Test staging install:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/hanzoai/agents/main/scripts/install.sh | bash -s -- --staging
   ~/.hanzo-agents-staging/bin/hanzo-agents --version
   ```
...
2. Verify:
   ...
   - [ ] `npm install @hanzo-agents/sdk` gets new version
   - [ ] GitHub release NOT marked as "Pre-release"
   - [ ] Docker image tagged `vX.Y.Z` and `latest`
3. Test `install.sh`:
   ```bash
   curl -fsSL https://hanzo-agents.ai/install.sh | bash
   ~/.hanzo-agents/bin/hanzo-agents --version
   ```
...
| npm @next | `npm unpublish @hanzo-agents/sdk@X.Y.Z-rc.N` (within 72 hours) or publish new rc |
...
- `https://hanzo-agents.ai/install.sh` (handles both production and staging via `--staging` flag)
- `https://hanzo-agents.ai/uninstall.sh`

**Options:**

1. **GitHub Raw URLs (Temporary):**
   ```
   https://raw.githubusercontent.com/hanzoai/agents/main/scripts/install.sh
   ```
...
2. Verify it matches the binary hash:
   ```bash
   sha256sum hanzo-agents-linux-amd64
   ```
...
- GitHub Issues: https://github.com/hanzoai/agents/issues
