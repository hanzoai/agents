...
This chart installs the Hanzo Agents control plane and optional demo agents.
...
```bash
helm upgrade --install hanzo-agents deployments/helm/hanzo-agents \
  -n hanzo-agents --create-namespace \
...
```bash
kubectl -n hanzo-agents port-forward svc/hanzo-agents-control-plane 8080:8080
```
