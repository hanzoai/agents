...
// notePayload represents the JSON payload sent to the HanzoAgents server.
...
// Note sends a progress/status message to the HanzoAgents server.
// This is useful for debugging and tracking agent execution progress
// in the HanzoAgents UI.
//
...
// Notef sends a formatted progress/status message to the HanzoAgents server.
...
func (a *Agent) sendNote(ctx context.Context, message string, tags []string) {
	// Check if HanzoAgents URL is configured
	baseURL := strings.TrimSpace(a.cfg.HanzoAgentsURL)
	if baseURL == "" {
	...
	// Build UI API URL (notes go to /v1/ui, not /v1)
	uiAPIURL := strings.Replace(baseURL, "/v1", "/v1/ui", 1)
	if !strings.Contains(uiAPIURL, "/v1/ui") {
		// If no /v1 was found, append /v1/ui
		uiAPIURL = strings.TrimSuffix(baseURL, "/") + "/v1/ui"
	}
