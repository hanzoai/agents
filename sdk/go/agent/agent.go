...
// ExecutionContext captures the headers HanzoAgents sends with each execution request.
...
type Config struct {
	...
	// HanzoAgentsURL is the base URL of the HanzoAgents control plane server.
	...
	// Format: a valid HTTP/HTTPS URL, e.g. "https://hanzo-agents.example.com".
	HanzoAgentsURL string
...
func New(cfg Config) (*Agent, error) {
	...
	if strings.TrimSpace(cfg.HanzoAgentsURL) != "" {
		c, err := client.New(cfg.HanzoAgentsURL, client.WithHTTPClient(httpClient), client.WithBearerToken(cfg.Token))
		if err != nil {
			return nil, err
		}
		a.client = c
...
// Initialize registers the agent with the HanzoAgents control plane without starting a listener.
func (a *Agent) Initialize(ctx context.Context) error {
	...
	if a.client == nil {
		return errors.New("HanzoAgentsURL is required when running in server mode")
	}
...
func (a *Agent) registerNode(ctx context.Context) error {
	...
	a.logger.Printf("node %s registered with HanzoAgents", a.cfg.NodeID)
	return nil
}
...
func (a *Agent) handleReasoner(w http.ResponseWriter, r *http.Request) {
	...
	if a.cfg.DeploymentType != "serverless" && execCtx.ExecutionID != "" && strings.TrimSpace(a.cfg.HanzoAgentsURL) != "" {
...
func (a *Agent) sendExecutionStatus(executionID string, payload map[string]any) error {
	base := strings.TrimSpace(a.cfg.HanzoAgentsURL)
	if executionID == "" || base == "" {
...
// Call invokes another reasoner via the HanzoAgents control plane, preserving execution context.
func (a *Agent) Call(ctx context.Context, target string, input map[string]any) (map[string]any, error) {
	if strings.TrimSpace(a.cfg.HanzoAgentsURL) == "" {
		return nil, errors.New("HanzoAgentsURL is required to call other reasoners")
	}
