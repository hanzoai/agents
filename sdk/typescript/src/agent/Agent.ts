...
export class Agent {
  ...
  discover(options?: DiscoveryOptions) {
    return this.controlPlaneClient.discoverCapabilities(options);
  }
  ...
  getWorkflowReporter(metadata: ExecutionMetadata) {
    return new WorkflowReporter(this.controlPlaneClient, {
      executionId: metadata.executionId,
      runId: metadata.runId,
  ...
  async serve(): Promise<void> {
    ...
    await this.controlPlaneClient.heartbeat('starting');
  ...
  async call(target: string, input: any) {
    ...
    if (!agentId || agentId === this.config.nodeId) {
      ...
      const emitEvent = async (status: 'running' | 'succeeded' | 'failed', payload: any) => {
        await this.controlPlaneClient.publishWorkflowEvent({
    ...
    return this.controlPlaneClient.execute(target, input, {
  ...
  private async registerWithControlPlane() {
    try {
      ...
      await this.controlPlaneClient.register({
  ...
  private startHeartbeat() {
    ...
    const tick = async () => {
      try {
        await this.controlPlaneClient.heartbeat('ready');
      } catch (err) {
