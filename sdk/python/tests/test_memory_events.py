...
from hanzo_agents.memory_events import (
    PatternMatcher,
    EventSubscription,
    MemoryEventClient,
)
from hanzo_agents.types import MemoryChangeEvent
...
def test_memory_event_client_subscription_and_unsubscribe(monkeypatch):
    ...
    client = MemoryEventClient("http://hanzo_agents", ctx)
...
async def test_memory_event_client_connect_builds_ws_url(monkeypatch):
    ...
    monkeypatch.setattr("hanzo_agents.memory_events.websockets.connect", fake_connect)
    ...
    assert record["url"].startswith("ws://hanzo_agents")
...
def test_websockets_version_detection():
    ...
    from hanzo_agents.memory_events import _WEBSOCKETS_MAJOR, _HEADERS_KWARG
...
async def test_connect_passes_correct_headers_kwarg(monkeypatch):
    ...
    from hanzo_agents.memory_events import _HEADERS_KWARG
...
async def test_connect_does_not_block_startup_on_failure(monkeypatch):
    ...
    monkeypatch.setattr("hanzo_agents.memory_events.websockets.connect", failing_connect)
