...
from hanzo_agents.decorators import reasoner, _execute_with_tracking
from hanzo_agents.execution_context import ExecutionContext
from hanzo_agents.agent_registry import set_current_agent, clear_current_agent
...
async def test_execute_with_tracking_success(monkeypatch):
    ...
    monkeypatch.setattr("hanzo_agents.decorators._send_workflow_start", record_start)
    monkeypatch.setattr(
        "hanzo_agents.decorators._send_workflow_completion", record_complete
    )
...
async def test_execute_with_tracking_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "hanzo_agents.decorators._send_workflow_start", lambda *a, **k: asyncio.sleep(0)
    )
    monkeypatch.setattr("hanzo_agents.decorators._send_workflow_error", record_error)
