...
from hanzo_agents.decorators import reasoner as tracked_reasoner
...
async def test_agent_reasoner_routing_and_workflow(monkeypatch):
    agent, hanzo_agents_client = create_test_agent(
    ...
    # Disable hanzo_agents_server to prevent async callback execution
    agent.hanzo_agents_server = None
    ...
    await agent.hanzo_agents_handler.register_with_hanzo_agents_server(port=9100)
    assert hanzo_agents_client.register_calls
    registration = hanzo_agents_client.register_calls[-1]
...
async def test_agent_reasoner_without_parentheses(monkeypatch):
    ...
    assert response.status_code == 200
    assert response.json() == {"message": "hello Hanzo Agents"}
...
async def test_agent_skill_without_parentheses(monkeypatch):
    ...
    assert response.status_code == 200
    assert response.json() == {"value": "HANZO_AGENTS"}
...
async def test_callback_url_precedence_and_env(monkeypatch):
    ...
    await explicit_agent.hanzo_agents_handler.register_with_hanzo_agents_server(port=9200)
    ...
    await env_agent.hanzo_agents_handler.register_with_hanzo_agents_server(port=9300)
