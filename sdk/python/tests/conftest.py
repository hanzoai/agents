# Pytest configuration and fixtures for Hanzo Agents SDK tests
"""
...
from hanzo_agents.types import AIConfig, MemoryConfig
...
def mock_container_detection(monkeypatch):
    ...
    Example:
        def test_callback_prefers_env(mock_container_detection):
            ...
            ...
    """
    from hanzo_agents import agent as agent_mod
...
def fake_server(monkeypatch, request):
    ...
    app = FastAPI(title="Hanzo Agents Fake Server")
    ...
    async def execute_target(target: str, payload: Dict[str, Any]):
        ...
        result = {
            ...
            "metadata": {
                ...
                "hanzo_agents_request_id": "req_" + uuid.uuid4().hex[:8],
...
@pytest.fixture
def dummy_headers():
    """Baseline execution headers consumed by memory/hanzo_agents client tests."""
    return {
        "X-Workflow-ID": "wf-test",
        "X-Execution-ID": "exec-test",
        "X-Hanzo Agents-Request-ID": "req-test",
    }
