...
def _write_hanzo_agents_config(config_path: Path, db_path: Path, kv_path: Path) -> None:
...
def hanzo_agents_binary(tmp_path_factory: pytest.TempPathFactory) -> Path:
    repo_root = Path(__file__).resolve().parents[4]
    hanzo_agents_go_root = repo_root / "apps" / "platform" / "hanzo_agents"
    if not hanzo_agents_go_root.exists():
        pytest.skip("Hanzo Agents server sources not available in this checkout")
    build_dir = tmp_path_factory.mktemp("hanzo_agents-server-bin")
    binary_name = (
        "hanzo_agents-test-server.exe" if os.name == "nt" else "hanzo_agents-test-server"
    )
    binary_path = build_dir / binary_name

    releases_dir = hanzo_agents_go_root / "dist" / "releases"
    ...
    if os_part:
        candidate = releases_dir / f"hanzo_agents-{os_part}-{arch_part}"
        if candidate.exists():
            prebuilt_path = candidate
        elif os_part == "darwin":
            universal = releases_dir / "hanzo_agents-darwin-arm64"
            if universal.exists():
    ...
    build_cmd = ["go", "build", "-o", str(binary_path), "./cmd/hanzo_agents"]
    env = os.environ.copy()
    env["GOCACHE"] = str(tmp_path_factory.mktemp("go-cache"))
    env["GOMODCACHE"] = str(tmp_path_factory.mktemp("go-modcache"))
    subprocess.run(build_cmd, check=True, cwd=hanzo_agents_go_root, env=env)
    return binary_path
...
def hanzo_agents_server(
    ...
    hanzo_agents_home = Path(tmp_path_factory.mktemp("hanzo_agents-home"))
    data_dir = hanzo_agents_home / "data"
    data_dir.mkdir(parents=True, exist_ok=True)

    db_path = data_dir / "hanzo_agents.db"
    kv_path = data_dir / "hanzo_agents.bolt"
    config_path = hanzo_agents_home / "hanzo_agents.yaml"

    _write_hanzo_agents_config(config_path, db_path, kv_path)
    ...
    env.update(
        {
            "HANZO_AGENTS_HOME": str(hanzo_agents_home),
            "HANZO_AGENTS_STORAGE_MODE": "local",
        }
    )

    cmd = [
        str(hanzo_agents_binary),
        "server",
        "--backend-only",
        "--port",
        str(port),
    ...
    log_path = hanzo_agents_home / "hanzo_agents.log"
    ...
    try:
        ...
        while time.time() < deadline:
            if process.poll() is not None:
                raise RuntimeError("Hanzo Agents server exited before becoming healthy")
            try:
