...
async def _wait_for_node(
    ...
    raise AssertionError(f"Node {node_id} did not appear in Hanzo Agents registry")
...
@pytest.mark.asyncio
async def test_agent_registration_and_status_propagation(hanzo_agents_server, run_agent):
    agent = Agent(
        node_id="integration-agent-status",
        hanzo_agents_server=hanzo_agents_server.base_url,
        dev_mode=True,
    ...
    await agent.hanzo_agents_handler.register_with_hanzo_agents_server(runtime.port)
    assert agent.hanzo_agents_connected is True

    async with httpx.AsyncClient(
        base_url=hanzo_agents_server.base_url, timeout=5.0
    ) as client:
        ...
        await agent.hanzo_agents_handler.send_enhanced_heartbeat()
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_reasoner_execution_roundtrip(hanzo_agents_server, run_agent):
    agent = Agent(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_app_ctx_available_during_execution(hanzo_agents_server, run_agent):
