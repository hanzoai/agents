...
from hanzo_agents.did_manager import DIDManager
from hanzo_agents.vc_generator import VCGenerator
from hanzo_agents.mcp_client import MCPClientRegistry
from hanzo_agents.mcp_manager import MCPManager
from hanzo_agents.memory import MemoryClient, MemoryInterface
from hanzo_agents.memory_events import MemoryEventClient
from hanzo_agents.logger import log_debug, log_error, log_info, log_warn
from hanzo_agents.router import AgentRouter
from hanzo_agents.connection_manager import ConnectionManager
from hanzo_agents.types import (
    AgentStatus,
    AIConfig,
    DiscoveryResult,
    MemoryConfig,
)
from hanzo_agents.multimodal_response import MultimodalResponse
from hanzo_agents.async_config import AsyncConfig
from hanzo_agents.async_execution_manager import AsyncExecutionManager
from hanzo_agents.pydantic_utils import convert_function_args, should_convert_args
...
class Agent(FastAPI):
    """
    Hanzo Agents Agent - FastAPI subclass for creating AI agent nodes.

    The Agent class is the core component of the Hanzo Agents SDK that enables developers to create
    intelligent agent nodes. It inherits from FastAPI to provide HTTP endpoints and integrates
    with the Hanzo Agents ecosystem for distributed AI workflows.

    Key Features:
    - Decorator-based reasoner and skill registration
    - Cross-agent communication via the Hanzo Agents execution gateway
    ...
    def __init__(
        self,
        node_id: str,
        hanzo_agents_server: str = "http://localhost:8080",
        version: str = "1.0.0",
        ...
        **kwargs,
        ...
        """
        Initialize a new Hanzo Agents Agent instance.

        Sets log level to DEBUG if dev_mode is True, else INFO.
        """
        # Set logging level based on dev_mode
        from hanzo_agents.logger import set_log_level
        ...
        (deterministic functions) while integrating with the Hanzo Agents ecosystem for distributed
        ...
        Args:
            node_id (str): Unique identifier for this agent node. Used for routing and
                          ...
                          within your Hanzo Agents ecosystem.
            hanzo_agents_server (str, optional): URL of the Hanzo Agents server for registration and
            ...
            callback_url (str, optional): Explicit callback URL for Hanzo Agents server to reach this agent.
            ...
            api_key (str, optional): API key for authenticating with the Hanzo Agents control plane.
        ...
        Example:
            ...
            app = Agent(
                ...
                hanzo_agents_server="https://hanzo_agents.company.com",
                version="2.1.0",
                ai_config=AIConfig(
                    provider="openai",
                    model="gpt-4",
        ...
        self.hanzo_agents_server = hanzo_agents_server
        ...
        self.hanzo_agents_connected = False
        self.auto_register = (
        ...
        # Add connection management for resilient Hanzo Agents server connectivity
        ...
        # Setup standard Hanzo Agents routes and memory event listeners
        self.server_handler.setup_hanzo_agents_routes()
        ...
        max_calls_env = os.getenv("HANZO_AGENTS_AGENT_MAX_CONCURRENT_CALLS")
        if max_calls_env:
            try:
                ...
                log_warn(
                    f"Invalid HANZO_AGENTS_AGENT_MAX_CONCURRENT_CALLS='{max_calls_env}', defaulting to {default_limit}"
                )
        else:
            self._max_concurrent_calls = default_limit
    ...
    def handle_serverless(self, event: dict, adapter: Optional[Callable] = None) -> dict:
        ...
        Special Endpoints:
            - /discover: Returns agent metadata for Hanzo Agents server registration
        ...
        if path == "/discover" or path.endswith("/discover") or action == "discover":
            # Return agent metadata for Hanzo Agents server registration
            return self._handle_discovery()

        # Auto-register with Hanzo Agents if needed (for execution requests)
        if self.auto_register and not self.hanzo_agents_connected:
            try:
                # Attempt registration (non-blocking)
                self.hanzo_agents_handler._register_agent()
                self.hanzo_agents_connected = True
            except Exception as e:
                if self.dev_mode:
    ...
    def _handle_discovery(self) -> dict:
        ...
        for automatic registration with the Hanzo Agents server.
    ...
    def _initialize_did_system(self):
        ...
        try:
            ...
            self.did_manager = DIDManager(
                self.hanzo_agents_server, self.node_id, self.api_key
            )

            # Initialize VC Generator
            self.vc_generator = VCGenerator(self.hanzo_agents_server, self.api_key)
    ...
    def _apply_discovery_response(self, payload: Optional[Dict[str, Any]]) -> None:
        """Update agent networking state from Hanzo Agents discovery response."""
        ...
        if resolved and resolved != self.base_url:
            log_debug(f"Applying resolved callback URL from Hanzo Agents: {resolved}")
    ...
    def _setup_hanzo_agents_routes(self):
        """Delegate to server handler for route setup"""
        return self.server_handler.setup_hanzo_agents_routes()
    ...
    def reasoner(
        ...
        *,
        ...
        It automatically handles input/output schema generation and integrates with the Hanzo Agents's AI capabilities.

        Args:
            path (str, optional): The API endpoint path for this reasoner. Defaults to /reasoners/{function_name}.
            name (str, optional): Explicit Hanzo Agents registration ID. Defaults to the function name.
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def endpoint(request: Request):
                ...
                if execution_id_header and self.hanzo_agents_server:
                    asyncio.create_task(
            ...
            async def tracked_func(*args, **kwargs):
                ...
                if current_context:
                    ...
                    from hanzo_agents.decorators import _execute_with_tracking
    ...
    async def _post_execution_status(
        ...
        if not self.client:
            log_error("Hanzo Agents client unavailable; cannot send status updates")
            return
    ...
    def _build_execution_callback_url(self, execution_id: str) -> Optional[str]:
        if not self.hanzo_agents_server or not execution_id:
            return None
        return (
    ...
    def skill(
        ...
        *,
        ...
        - Enables cross-agent communication via the Hanzo Agents execution gateway
        ...
        Returns:
            Callable: The decorated function with enhanced Hanzo Agents integration.
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def _run_sync_skill(*args, **kwargs):
                current_context = get_current_context()
                if not current_context or not self.hanzo_agents_server:
    ...
    async def call(self, target: str, *args, **kwargs) -> dict:
        """
        Initiates a cross-agent call to another reasoner or skill via the Hanzo Agents execution gateway.

        This method allows agents to seamlessly communicate and utilize reasoners/skills
        deployed on other agent nodes within the Hanzo Agents ecosystem. It properly propagates
        ...
        **Return Type**: Always returns JSON/dict objects, similar to calling any REST API.
        ...
        if self.dev_mode:
            from hanzo_agents.execution_context import get_current_context
        ...
        # DISABLED: Same-agent call detection - Force all calls through Hanzo Agents server
        # This ensures all app.call() requests go through the Hanzo Agents server for proper
        # workflow tracking, execution context, and distributed processing
        from hanzo_agents.logger import log_debug

        log_debug(f"Cross-agent call to: {target}")

        # Check if Hanzo Agents server is available for cross-agent calls
        if not self.hanzo_agents_connected:
            from hanzo_agents.logger import log_warn

            log_warn(
                f"Hanzo Agents server unavailable - cannot make cross-agent call to {target}"
            )
            raise Exception(
        ...
        try:
            async with self._limit_outbound_calls():
                ...
                use_async_execution = (
                    ...
                    and self.hanzo_agents_connected
                )
    ...
    async def _get_async_execution_manager(self) -> AsyncExecutionManager:
        ...
        if self._async_execution_manager is None:
            ...
            self._async_execution_manager = AsyncExecutionManager(
                base_url=self.hanzo_agents_server, config=self.async_config
            )
            # Start the manager
    ...
    def note(self, message: str, tags: List[str] = None) -> None:
        ...
        This method sends a note to the Hanzo Agents server asynchronously without blocking
        the current execution. The note is automatically associated with the current
        execution context and can be viewed in the Hanzo Agents UI for debugging and monitoring.
    ...
    def _emit_workflow_event_sync(
        ...
        *,
        ...
        if not self.hanzo_agents_server:
            return
    ...
    def discover(
        ...
        if not self.client:
            raise RuntimeError("Hanzo Agents client is not configured")
    ...
    def serve(  # pragma: no cover - requires full server runtime integration
        ...
        **kwargs,
        ...
        """
        Start the agent node server with intelligent port management and Hanzo Agents integration.

        This method launches the agent as a FastAPI server that can receive reasoner and skill
        requests from other agents via the Hanzo Agents execution gateway. It handles automatic
        registration with the Hanzo Agents server, heartbeat management, and graceful shutdown.

        The server provides:
        - RESTful endpoints for all registered reasoners and skills
        - Health check endpoints for monitoring
        - MCP server status and management endpoints
        - Automatic Hanzo Agents server registration and heartbeat
        ...
        Args:
            ...
            heartbeat_interval (int): The interval in seconds for sending heartbeats to the Hanzo Agents server.
        ...
        Integration with Hanzo Agents:
            - Automatically registers with Hanzo Agents server on startup
            - Sends periodic heartbeats to maintain connection
            - Receives execution requests via Hanzo Agents's routing system
