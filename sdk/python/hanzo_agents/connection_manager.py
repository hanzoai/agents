"""
Hanzo Agents SDK Connection Manager

Provides resilient connection handling for Hanzo Agents server connectivity.
...
from hanzo_agents.logger import log_debug, log_info, log_warn, log_error


class ConnectionState(Enum):
    """Connection states for Hanzo Agents server connectivity"""

    DISCONNECTED = "disconnected"
    CONNECTING = "connecting"
    CONNECTED = "connected"
    RECONNECTING = "reconnecting"
    DEGRADED = "degraded"  # Running locally without Hanzo Agents server
...
class ConnectionManager:
    """
    Manages resilient connections to Hanzo Agents server with automatic reconnection,
    graceful degradation, and health monitoring.

    Uses a simple, consistent retry interval to ensure immediate reconnection
    when Hanzo Agents server becomes available.
    """
    ...
    async def _attempt_connection(self) -> bool:
        """
        Attempt to connect to Hanzo Agents server.
        ...
        try:
            ...
            log_debug("Attempting connection to Hanzo Agents server")

            # Try to register with Hanzo Agents server - suppress verbose error logging
            import logging
    ...
    async def _health_check_loop(self):
        ...
        while not self._shutdown_requested and self.state == ConnectionState.CONNECTED:
            try:
                ...
                success = await self.agent.hanzo_agents_handler.send_enhanced_heartbeat()
    ...
    def _on_connection_success(self):
        ...
        log_info("Connected to Hanzo Agents server")
    ...
    def _on_connection_failure(self):
        ...
        log_warn("Hanzo Agents server unavailable - running in degraded mode")
    ...
    def is_connected(self) -> bool:
        """Check if currently connected to Hanzo Agents server"""
