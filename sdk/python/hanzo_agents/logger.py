"""
Hanzo Agents SDK Logging Utility

This module provides a centralized logging system for the Hanzo Agents SDK that:
...
class LogLevel(Enum):
    """Log levels for Hanzo Agents SDK"""
...
class Hanzo AgentsLogger:
    ...
    def __init__(self, name: str = "hanzo_agents"):
        self.logger = logging.getLogger(name)
        self._setup_logger()

        # Configuration from environment variables - default to WARNING (only important events)
        self.log_level = os.getenv("HANZO_AGENTS_LOG_LEVEL", "WARNING").upper()
        self.truncate_length = int(os.getenv("HANZO_AGENTS_LOG_TRUNCATE", "200"))
        self.show_payloads = (
            os.getenv("HANZO_AGENTS_LOG_PAYLOADS", "false").lower() == "true"
        )
        self.show_tracking = (
            os.getenv("HANZO_AGENTS_LOG_TRACKING", "false").lower() == "true"
        )
        self.show_fire = os.getenv("HANZO_AGENTS_LOG_FIRE", "false").lower() == "true"
    ...
    def _format_payload(self, payload: Any) -> str:
        ...
        if not self.show_payloads:
            return "[payload hidden - set HANZO_AGENTS_LOG_PAYLOADS=true to show]"
    ...
    def track(self, message: str, **kwargs):
        """Log tracking messages (controlled by HANZO_AGENTS_LOG_TRACKING)"""
        if self.show_tracking:
            self.logger.debug(f"🔍 TRACK: {self._truncate_message(message)}")

    def fire(self, message: str, payload: Optional[Any] = None, **kwargs):
        """Log fire-and-forget workflow messages (controlled by HANZO_AGENTS_LOG_FIRE)"""
        if self.show_fire:
