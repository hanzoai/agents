...
from hanzo_agents.logger import (
    log_heartbeat,
    log_debug,
    log_warn,
    log_error,
...
class Hanzo AgentsHandler:
    ...
    This class encapsulates all Hanzo Agents server communication functionality including:
    - Agent registration with Hanzo Agents server
    ...
    def __init__(self, agent_instance):
        """
        Initialize the Hanzo Agents handler with a reference to the agent instance.
    ...
    async def register_with_hanzo_agents_server(self, port: int):
        """Register this agent node with Hanzo Agents server"""
        # Import the callback URL resolution function
        from hanzo_agents.agent import (
        ...
        try:
            log_debug(
                f"Attempting to register with Hanzo Agents server at {self.agent.hanzo_agents_server}"
            )
            ...
            if success:
                ...
                log_success(
                    f"Registered node '{self.agent.node_id}' with Hanzo Agents server"
                )
                self.agent.hanzo_agents_connected = True

                # Attempt DID registration after successful Hanzo Agents registration
                ...
                self.agent.hanzo_agents_connected = False

        except Exception as e:
            self.agent.hanzo_agents_connected = False
            if self.agent.dev_mode:
                log_warn(f"Hanzo Agents server not available: {e}")
                log_setup("Running in development mode - agent will work standalone")
                log_info(
                    f"To connect to Hanzo Agents server, start it at {self.agent.hanzo_agents_server}"
                )
            else:
                log_error(f"Failed to register with Hanzo Agents server: {e}")
                if (
    ...
    def send_heartbeat(self):
        """Send heartbeat to Hanzo Agents server"""
        if not self.agent.hanzo_agents_connected:
            return  # Skip heartbeat if not connected to Hanzo Agents
    ...
    def heartbeat_worker(
        ...
        if not self.agent.hanzo_agents_connected:
            log_heartbeat(
                "Heartbeat worker skipped - not connected to Hanzo Agents server"
            )
            return
    ...
    async def notify_shutdown(self) -> bool:
        """
        Notify Hanzo Agents server of graceful shutdown.
    ...
    def setup_fast_lifecycle_signal_handlers(
        ...
        - Only intercepts SIGTERM to mark the agent offline and notify Hanzo Agents immediately.
        - Leaves SIGINT (Ctrl+C) to uvicorn so its shutdown hooks run and resources are cleaned up.
        """

        def signal_handler(signum: int, frame) -> None:
            """Handle SIGTERM: mark offline, notify Hanzo Agents, then re-emit the signal for default handling."""
            ...
            # Best-effort immediate notification to Hanzo Agents
            try:
    ...
    async def register_with_fast_lifecycle(
        ...
        """
        from hanzo_agents.agent import _build_callback_candidates, _resolve_callback_url
        ...
        try:
            if self.agent.dev_mode:
                log_info(
                    f"Fast registration with Hanzo Agents server at {self.agent.hanzo_agents_server}"
                )
