...
import uvicorn
from hanzo_agents.agent_utils import AgentUtils
from hanzo_agents.logger import log_debug, log_error, log_info, log_success, log_warn
from hanzo_agents.utils import get_free_port
from fastapi import Request
from fastapi.routing import APIRoute


class AgentServer:
    """Server management functionality for Hanzo Agents Agent"""
    ...
    def setup_hanzo_agents_routes(self):
        """Setup standard routes that Hanzo Agents server expects"""
        ...
        async def shutdown_agent(request: Request):
            ...
            This endpoint allows the Hanzo Agents server to request a graceful shutdown
            ...
            try:
                ...
                # Set shutdown status
                from hanzo_agents.agent import AgentStatus

                self.agent._shutdown_requested = True
                self.agent._current_status = AgentStatus.OFFLINE

                # Notify Hanzo Agents server of shutdown initiation
                try:
        ...
        async def mcp_health():
            """Get MCP health information in the format expected by Hanzo Agents server"""
            if not self.agent.mcp_manager:
        ...
        async def restart_mcp_server_alt(alias: str):
            """Alternative restart endpoint for Hanzo Agents server compatibility"""
    ...
    async def _graceful_shutdown(self, timeout_seconds: int = 30):
        ...
        try:
            ...
            try:
                if (
                    hasattr(self.agent, "hanzo_agents_handler")
                    and self.agent.hanzo_agents_handler
                ):
                    self.agent.hanzo_agents_handler.stop_heartbeat()
                    if self.agent.dev_mode:
            ...
            try:
                from hanzo_agents.agent_registry import clear_current_agent
    ...
    def serve(
        ...
        **kwargs,
        ...
        This method implements smart port resolution that seamlessly works with Hanzo Agents CLI
        or standalone execution. The port selection priority is:
        1. Explicit port parameter (highest priority)
        2. PORT environment variable (Hanzo Agents CLI integration)
        ...
        if port is None:
            # Check for Hanzo Agents CLI integration via environment variable
            ...
            if env_port and env_port.isdigit():
                ...
                if AgentUtils.is_port_available(suggested_port):
                    port = suggested_port
                    if self.agent.dev_mode:
                        log_debug(f"Using port from Hanzo Agents CLI: {port}")
                else:
                    # Hanzo Agents CLI suggested port is taken, find next available
                    try:
                        port = get_free_port(start_port=suggested_port)
                        if self.agent.dev_mode:
                            log_debug(
                                f"Hanzo Agents CLI port {suggested_port} taken, using {port}"
                            )
                    except RuntimeError:
                        port = get_free_port()  # Fallback to default range
                        if self.agent.dev_mode:
                            log_debug(f"Using fallback port: {port}")
            elif auto_port or os.getenv("HANZO_AGENTS_AUTO_PORT") == "true":
        ...
        self.agent.hanzo_agents_handler.start_heartbeat(heartbeat_interval)
        ...
        self.agent.hanzo_agents_handler.setup_fast_lifecycle_signal_handlers()

        # Add startup event handler for resilient lifecycle
        @self.agent.on_event("startup")
        async def startup_resilient_lifecycle():
            """Resilient lifecycle startup: connection manager handles Hanzo Agents server connectivity"""

            # Initialize connection manager
            from hanzo_agents.connection_manager import (
                ConnectionConfig,
                ConnectionManager,
            )
            ...
            config = ConnectionConfig(
                retry_interval=10.0,  # Check every 10 seconds for Hanzo Agents server
            ...
            def on_connected():
                if self.agent.dev_mode:
                    log_info(
                        "Connected to Hanzo Agents server - full functionality available"
                    )
                # Kick a heartbeat immediately so the control plane renews the lease
                try:
                    asyncio.create_task(
                        self.agent.hanzo_agents_handler.send_enhanced_heartbeat()
                    )
                except RuntimeError:
                ...
                if (
                    ...
                    self.agent._heartbeat_task = asyncio.create_task(
                        self.agent.hanzo_agents_handler.enhanced_heartbeat_loop(
                            heartbeat_interval
                        )
                    )
            ...
            def on_disconnected():
                if self.agent.dev_mode:
                    log_warn("Hanzo Agents server disconnected - running in local mode")
            ...
            # These work independently of Hanzo Agents server connection
            ...
            if connected:
                if self.agent.dev_mode:
                    log_info("Agent started with Hanzo Agents server connection")
            else:
                if self.agent.dev_mode:
                    log_info(
                        "Agent started in local mode - will connect to Hanzo Agents server when available"
                    )
        ...
        async def shutdown_cleanup():
            ...
            if getattr(self.agent, "client", None):
                try:
                    ...
                    if self.agent.dev_mode:
                        log_error(f"Hanzo Agents client shutdown error: {e}")
